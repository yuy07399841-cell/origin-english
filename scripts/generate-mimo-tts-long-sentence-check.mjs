...
const OUTPUT_DIR = new URL('../artifacts/tts-audition-long-sentence/', import.meta.url)
const AUDIO_PATH = new URL('mia-long-sentence.wav', OUTPUT_DIR)
...
try {
  ...
  throw new Error('The long-sentence summary already exists. Refusing to overwrite it.')
} catch (error) {
...
if (!response.ok) {
  throw new Error(`MiMo long-sentence TTS request failed with status ${response.status}. No retry was attempted.`)
}

const audio = payload?.choices?.[0]?.message?.audio
if (typeof audio?.data !== 'string' || audio.data.length === 0) {
  throw new Error('MiMo long-sentence TTS returned no audio. No retry was attempted.')
}
...
if (!isWav) {
  throw new Error('MiMo long-sentence TTS returned an invalid WAV payload. No retry was attempted.')
}
